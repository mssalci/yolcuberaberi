...
export default function YolculukDetay() {
  ...
  const [yolculukSahibiAdSoyad, setYolculukSahibiAdSoyad] = useState("");
  ...
  useEffect(() => {
    const fetchYolculuk = async () => {
      try {
        ...
        if (snap.exists()) {
          const yolculukData = { id: snap.id, ...snap.data() };
          setYolculuk(yolculukData);

          // Yolculuk sahibinin adını/soyadını çek
          const kullaniciRef = doc(db, "kullanicilar", yolculukData.kullaniciId);
          const kullaniciSnap = await getDoc(kullaniciRef);
          if (kullaniciSnap.exists()) {
            setYolculukSahibiAdSoyad(kullaniciSnap.data().adSoyad || "Bilinmiyor");
          }
        }
      } catch (err) {
    ...
    const fetchEslesmeler = async () => {
      try {
        ...
        for (let docSnap of snapshot.docs) {
          ...
          // Teklif verenin adını/soyadını çek
          const teklifVerenRef = doc(db, "kullanicilar", docSnap.data().teklifVerenId);
          ...
          data.push({
            ...
            teklifVerenAd: teklifVerenSnap.exists() ? teklifVerenSnap.data().adSoyad : "Bilinmiyor"
          });
        }
        setEslesmeler(data);
      } catch (err) {
  ...
  return (
    ...
    <div className="space-y-3">
      ...
      <p className="text-gray-600 text-sm mb-1">Tarih: {yolculuk.tarih || "-"}</p>      <p className="text-gray-700">Yolculuk Sahibi: {yolculukSahibiAdSoyad || "-"}</p>
            ...
            {eslesmeler.map((eslesme) => (
              <li key={eslesme.id} className="p-3 bg-white rounded shadow flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Teklif Veren: {eslesme.teklifVerenAd}</p>
