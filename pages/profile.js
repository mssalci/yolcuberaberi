...
const fetchData = async () => { const userDoc = await getDoc(doc(db, "users", currentUser.uid)); if (userDoc.exists()) { setUserName(userDoc.data().name || ""); } const q = query(collection(db, "talepler"), where("uid", "==", currentUser.uid)); const taleplerSnapshot = await getDocs(q); const taleplerList = taleplerSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); setTaleplerim(taleplerList); const teklifSnap = await getDocs( query(collection(db, "teklifler"), where("teklifVerenUid", "==", currentUser.uid)) ); const teklifler = []; const eslesenler = []; for (let teklifDoc of teklifSnap.docs) { const data = teklifDoc.data(); const talepDoc = await getDoc(doc(db, "talepler", data.talepId)); const teklifData = { id: teklifDoc.id, ...data, talep: talepDoc.exists() ? talepDoc.data() : null, }; teklifler.push(teklifData); if (data.durum === "kabul edildi") { eslesenler.push(teklifData); } } const gelenTekliflerSnap = await getDocs( query(collection(db, "teklifler"), where("durum", "==", "kabul edildi")) ); for (let teklifDoc of gelenTekliflerSnap.docs) { const data = teklifDoc.data(); const talepDoc = await getDoc(doc(db, "talepler", data.talepId)); if (talepDoc.exists() && talepDoc.data().uid === currentUser.uid) { const teklifData = { id: teklifDoc.id, ...data, talep: talepDoc.data(), }; eslesenler.push(teklifData); } } setVerdigimTeklifler(teklifler); setEslesmeler(eslesenler); }; fetchData(); } }); return () => unsubscribe(); 
